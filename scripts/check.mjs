...
verify((homepage.match(/data-progress-link=/g) || []).length === 5, 'Five direct scene links are rendered inside the expandable navigation');
...
verify(homepage.includes('data-story-menu-close') && homepage.includes('data-story-nav-link') && homepage.includes('class="story-menu-wordmark"'), 'Story menu includes a large RV mark, selectable links, and a close control');
...
verify(stackStyles.includes('.story-menu-toggle') && stackStyles.includes('.story-menu-close {') && stackStyles.includes('min-width: 44px; min-height: 44px;') && stackStyles.includes('.story-menu-links a {') && stackStyles.includes('min-height: 3.5rem;'), 'Expandable menu controls and links meet touch-target sizing');
verify(stackStyles.includes('.home-page[data-story-menu-open="true"] .story-stage { transform: translate3d(var(--story-menu-width), 0, 0); }') && stackStyles.includes('.story-menu { width: min(92vw, 28rem); max-width: 92vw; }'), 'Menu pushes the desktop scene and stays near-full-width on compact screens');
...
verify(stackStyles.includes('@media (prefers-reduced-motion: reduce)') && stackStyles.includes('.technology-orbit-item.is-compact p { display: block !important; }') && stackStyles.includes('.story-stage--depth .stack-orbit-mascot img { max-height: 24rem !important; animation: none !important; }'), 'Reduced motion removes floating and makes every technology description available');
...
verify(animationSource.includes('IntersectionObserver') && animationSource.includes('data-progress-link'), 'Expandable scene links track the current section in normal-flow mode');
...
verify(animationSource.includes('navigateToStoryAnchor') && animationSource.includes('labelToScroll') && animationSource.includes("addEventListener('scrollend'") && animationSource.includes("link.closest('.site-header, .story-menu"), 'RV and expandable-menu links navigate to the matching narrative state');
verify(animationSource.includes('progressLinks') && animationSource.includes("setAttribute('aria-current', 'location')") && animationSource.includes("addEventListener('popstate'"), 'Scene navigation aria-current and browser history follow the active scene');
